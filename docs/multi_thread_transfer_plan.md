...
| 测试项 | 文件大小 | 预期行为 | 验证方法 |
|--------|----------|----------|----------|
| 小文件下载 | < 10MB | 单线程下载 | 控制台显示 `Using single-thread download mode` |
| 中文件下载 | 10-50MB | 2 分块下载 | 控制台显示 `Using chunked download mode, chunkCount: 2` |
| 大文件下载 | 50-200MB | 4 分块下载 | 控制台显示 `chunkCount: 4` |
| 超大文件下载 | > 200MB | 8 分块下载 | 控制台显示 `chunkCount: 8` |
| 进度显示 | 任意 | 实时更新进度条 | UI 进度条同步更新 |
| 下载完成 | 任意 | 文件完整 | 下载后校验 MD5 或文件大小 |
